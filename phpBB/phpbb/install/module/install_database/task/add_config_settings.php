...
class add_config_settings extends database_task
{
	...
	public function run()
	{
		$current_time = $this->install_config->get('install_time');
		if ($current_time === false)
		{
			$current_time = time();
			$this->install_config->set('install_time', $current_time);
		}
		...
		$total = count($updates);
		$sql_ary = array_slice($updates, $i);
		try {
			$stmt = $this->create_prepared_stmt(
				'UPDATE ' . $this->config_table . ' SET config_value = ? WHERE config_name = ?'
			);
